...
trait ActiveUserHelper
{
    ...
    private function calculateScore()
    {
        ...
        foreach ($this->users as $key => $user) {
            $this->users[$key]['score'] = ($this->users[$key]['topic_score'] ?? 0) + ($this->users[$key]['reply_score'] ?? 0);
            unset($this->users[$key]['topic_score']);
            unset($this->users[$key]['reply_score']);
        }
    }
    ...
    private function calculateTopicScore()
    {
        ...
        foreach ($topic_users as $value) {
            $this->users[$value->user_id]['topic_score'] = $value->topic_count * $this->topic_weight;
        }
    }

    private function calculateReplyScore()
    {
        // 从回复数据表里取出限定时间范围（）内，有发表过回复的用户
        ...
        foreach ($reply_users as $value) {
            $this->users[$value->user_id]['reply_score'] = $value->reply_count * $this->reply_weight;
        }
    }
